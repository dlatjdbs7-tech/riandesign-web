...
| PROJECTS | 현장관리 (신규/상담/견적 파이프라인 + 진행중 현장 위험도·공정률(하늘색)·수금률(연빨강) 카드, 수금액은 1~5차 분할입금(계약금/착수금/중도금1·2차/잔금)으로 관리, 공사기간은 공정표 최소~최대일 자동 반영, 카드의 제목·고객명·금액·담당·날짜 전부 인라인 수기 수정 가능 + 마감/취소·보류 목록, 현장 직접등록) | 구현 완료 |
| PROJECTS | 고객페이지 (작업지시서 기반 자동 생성 또는 수기로 직접 생성, 공개 링크로 현장사진 공유 `/project/[id]`. 입력 권한은 팀원권한의 "고객페이지" 메뉴 설정을 따름), AS관리 | 구현 완료 |
| PROJECTS | 견적서, 작업지시서, 거래명세서(파일 첨부 지원) — 사이드바에서 숨기고 시공관리 셀로만 접근 (팀원권한 설정은 유지) | 기본 CRUD 구현 |
| DAILY | 할일, 알림센터 (액션 필요 항목 모음), 근태·휴무(위치기반) | 구현 완료 |
| DAILY | 업무일지 | 구현 완료 |
| LIBRARY | 자재리스트, 자주 쓰는 문구 | 기본 CRUD 구현 |
| LIBRARY | 템플릿 목록 | 준비중 |
| PEOPLE | 팀원정보(가입승인, 구 "임직원"), 업체정보(구 "견적처"), 고객·우수회원(구 "고객관리" + ★ 우수회원 표시/토글), 임직원권한(구 "팀원권한", 대표 전용) — 각 페이지 하단에 해당 유형(팀원/업체/고객)으로 구분된 카테고리등록란 포함, 카테고리 메뉴는 사이드바에서 숨김 | 기본 CRUD 구현 |
