...
| PROJECTS | 캘린더 (작업지시서/AS/할일 일정 통합 월간 뷰), 현장관리 (상태별 칸반 보드 + 공정표) | 구현 완료 |
| PROJECTS | 고객페이지 (작업지시서별 공개 링크로 현장사진 공유, `/project/[id]`), AS관리 | 구현 완료 |
| PROJECTS | 견적서, 작업지시서, 거래명세서, 고객관리 | 기본 CRUD 구현 |
